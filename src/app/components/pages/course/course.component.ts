...
export class CourseComponent extends BasePageComponent implements OnInit {
  ...
  courseList: WritableSignal<Set<string>> = signal<Set<string>>(
    new Set<string>()
  );
  tags: WritableSignal<Set<string>> = signal<Set<string>>(new Set<string>());

  coursesFilter: WritableSignal<Set<string>> = signal<Set<string>>(
    new Set<string>()
  );
  ...
  ngOnInit(): void {
    this.courses().forEach((course) =>
      this.courseList().add(course.institution.trim())
    );
    this.courses().forEach((course) =>
  ...
  getCourseList = () =>
    Array.from(this.courseList().values()).sort((a, b) => (a > b ? 1 : -1));
  ...
  onClickIntitutionEvent(e: MouseEvent) {
    ...
    this.coursesFilter().has(id)
      ? this.coursesFilter().delete(id)
      : this.coursesFilter().add(id);
    this.selectInstitutionsFilter.set(
