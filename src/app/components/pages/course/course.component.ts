...
export class CourseComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.courses.forEach((course) =>
      this.institutions.add(course.institution.trim())
    )
    this.courses.forEach((course) =>
  ...
  onClickIntitutionEvent(e: MouseEvent) {
    ...
    this.institutionsFilter.has(id)
      ? this.institutionsFilter.delete(id)
      : this.institutionsFilter.add(id)
    this.selectInstitutionsFilter = Array.from(
      this.institutionsFilter.values()
    ).join(',')
